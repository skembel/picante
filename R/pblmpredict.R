pblmpredict <- function(x, tree1.w.novel = NULL, tree2.w.novel = NULL, predict.originals = FALSE) {
  ...
  if (predict.originals) {
    ...
    }
    {
      ...
      other.assocs <- seq_along(pairnames)
    ...
    if (is(tree1.w.novel)[1] == "phylo") {
      if (is.null(tree1.w.novel$edge.length)) {
        tree1.w.novel <- compute.brlen(tree1.w.novel, 1)
    ...
    if (is(tree2.w.novel)[1] == "phylo") {
      if (is.null(tree2.w.novel$edge.length)) {
        tree2.w.novel <- compute.brlen(tree2.w.novel, 1)
      } # If phylo has no given branch lengths
